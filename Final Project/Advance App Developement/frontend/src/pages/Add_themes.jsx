...
function Add_themes() {
  ...
  useEffect(() => {
    const response = axios.get("http://localhost:8080/admin/get-themes/"+user.email).then(
      (res) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = axios.post("http://localhost:8080/admin/add-theme/"+user.email,theme);
      console.log(response);
      setAddTheme(false);
